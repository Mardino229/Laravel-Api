...
@section('content')
    <div class="mt-8 sm:mt-16">
        ...
        <div class="flex flex-wrap items-center">
            <div class="">
                <a href="{{route("articles.create")}}" class="p-2 m-2 bg-green-600 text-white rounded-lg ">Create Article</a>
            </div>
            <div class="">
                <a href="{{route("category.create")}}" class="p-2 m-2 bg-green-600 text-white rounded-lg ">Create Category</a>
            </div>
            @foreach($categories as $category)
                <kbd class=" flex items-center gap-1 my-2 mx-2 px-2 py-1.5 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-200 rounded-lg dark:bg-gray-600 dark:text-gray-100 dark:border-gray-500">
                    {{$category->name}}
                    <a href="{{route("category.edit", $category)}}" class="cursor-pointer">
                        <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z"/>
                        </svg>
                    </a>
                    <form action="{{route("category.delete", $category)}}" class="" method="POST">
                        @csrf
                        <button type="submit">
        ...
        <div class="flex m-8 gap-4 flex-wrap">
            @foreach($articles as $article)
                <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <div class="px-5 pb-5">
                        <a href="#">
                            <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{{$article->name}}</h5>
                        </a>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{route("articles.edit", $article)}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Edit</a>
